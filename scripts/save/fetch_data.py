...
def compute_stock_descriptors(stock_data, dates, rf_series):
    ...
    for date in dates:
        ...
        last = hist_to_date.iloc[-1]
        daily_return = last['return']
        ...
        records.append({
            ...
            'size_raw': size_raw,
            'momentum_raw': last['rstr'] if 'rstr' in hist.columns else np.nan,
            'dastd_raw': last['dastd'] if 'dastd' in hist.columns else np.nan,
            'cmra_raw': last['cmra'] if 'cmra' in hist.columns else np.nan,
            'stom_raw': last['stom'] if 'stom' in hist.columns else np.nan,
            'stoq_raw': last['stoq'] if 'stoq' in hist.columns else np.nan,
            'stoa_raw': last['stoa'] if 'stoa' in hist.columns else np.nan,
            'btop_raw': btop_raw,
            'etop_raw': etop_raw,
            'cetop_raw': cetop_raw,
            'egro_raw': egro_raw,
...
def add_beta_and_hsigma(df):
    ...
    Uses cap-weighted market return.
    """
    print("\nComputing beta and HSIGMA...")
    ...
    for ticker, group in df.groupby('ticker'):
        ...
        # HSIGMA: EWM std of regression residuals
