...
def reg(self):
    ...
    Returns: factor_returns, specific_returns, pure_factor_exposures, R2_adj
    """
    W = np.diag(self.W)
    ...
    return factor_ret, specific_ret, pure_factor_exposure, R2_adj
...
class BarraModel:
    ...
    def run_cross_sectional_regression(self):
        ...
        self.R2 = pd.DataFrame(r2_values, columns=['R2_adj'], index=self.dates)
        self.specific_ret = specific_returns

        print(f"\n  Average adjusted R²: {self.R2['R2_adj'].mean():.4f}")
        return self.factor_ret, self.R2_adj
